...
void echo(int sd) {
	...
	while (1) {
		...
		if (n < 0) {
			...
			//uppercase(bufin);//转大写		
			sprintf(str, "%d ", sequence);
			sequence = sequence + 1;
			for (int i = strlen(bufin) + 1; i >= 0; i--)
			{
				bufin[i + strlen(str)] = bufin[i];
			}
			...
			sendto(sd, bufin, n + strlen(str + 1), 0, (struct sockaddr *)&remote, len);
		}
	}
}
...
int main() {
	int ld;
	struct sockaddr_in skaddr;//处理网络通信的地址
	socklen_t length;
